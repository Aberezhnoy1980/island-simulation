...
**Статус:** рабочее ядро симуляции (фазы, питание, размножение, стоп-условия, валидация конфига), CLI (в т.ч. `--scheduled`, `--render-map-every`, `--seed`), тесты на детерминизм и паритет режимов запуска; дальше — расширения (Unicode-глифы по видам, персистентность, доп. параллелизм по фазам).
...
Упрощённая консольная карта — `--render-map-every`; Unicode по видам — см. ТЗ (опционально).
