...
Приложение загружает конфиг, строит сетку (`Island` / `Location`), создаёт организмов через `OrganismFactory` (`Predator` / `Herbivore` / `Plant`) и раскладывает стартовые популяции по клеткам. Логика тиков (движение, питание, размножение) — в следующих инкрементах.
