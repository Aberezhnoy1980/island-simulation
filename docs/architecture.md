...
- **Многопоточность:** опционально `parallel` Stream по строкам сетки при планировании движения/роста; применение плана к острову — в одном потоке. Также в parallel-режиме распараллелены только безопасные cell-local проходы (`feeding` reset, starvation-pass `death`). Режим `--scheduled` — один поток планировщика, тики не параллелятся.
...
4. **Параллельное планирование и cell-local проходы (опционально)** — распараллеливаем только расчёты по строкам и операции, не меняющие связи между клетками; **применение** межклеточных изменений остаётся последовательным, чтобы не ловить гонки по общим коллекциям.
...
7. **Два UI-режима в консоли** — `stream` (append-only лог) и `live` (ANSI-перерисовка экрана) без вмешательства в бизнес-логику фаз.
